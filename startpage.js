...
const center = canvasFunctions.getCenter()
...
const height = (
    length.actualBoundingBoxAscent - length.actualBoundingBoxDescent)
...
mainContext.roundRect(
    center.x - length.width / 2  - length.width / 8, 
    center.y - height * 2, 
...
mainContext.fillText(
    word, 
    center.x - length.width / 2, 
    center.y + height / 2)

mainContext.strokeText(word, 
    center.x - length.width / 2, 
    center.y + height / 2)
 
const gombi = canvasFunctions.insertElement(
    center.x - length.width / 2  - length.width / 8, 
    center.y - height * 2, 
    length.width * 1.25, height * 4, 
    "button")
...
gombi.addEventListener("click", handle => {loadSelect("button")})
